...
from torch.distributed.fsdp.wrap import _or_policy, lambda_auto_wrap_policy, transformer_auto_wrap_policy, _module_wrap_policy


def init_fn(x: torch.nn.Module):
    """Make sure only the first rank will keep the model in memory.
    """
...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    print(f"[Actor] Default transformer cls names to wrap: {default_transformer_cls_names_to_wrap}")
    fsdp_transformer_layer_cls_to_wrap = config.get("transformer_layer_cls_to_wrap",
                                                    default_transformer_cls_names_to_wrap)
    print(f"[Actor] FSDP transformer layer cls to wrap: {fsdp_transformer_layer_cls_to_wrap}")
...
# openvla
def get_fsdp_wrap_policy_openvla(module, config=None, is_lora=False):
    ...
    # else: # will oom
    llm_fsdp_wrapping_policy = functools.partial(
            transformer_auto_wrap_policy, transformer_layer_cls={LlamaDecoderLayer}
        )
    policies.append(llm_fsdp_wrapping_policy)
...
# film-like value model
def get_fsdp_wrap_policy_film(module):
    """Get FSDP wrap policy for FiLM critic model.
    Returns:
        An FSDP wrapping policy function
    """
    from transformers.models.bert.modeling_bert import BertModel, BertLayer
    
    # Create a set to keep track of modules that have been wrapped
    # to prevent double-wrapping
    wrapped_modules = set()
    
    def film_module_policy_fn(module, recurse=False, **kwargs):
        # Check if module is already processed to avoid double wrapping
        module_id = id(module)
        if module_id in wrapped_modules:
            return False
            
        # Check if module is a BertLayer
        if isinstance(module, BertLayer):
            wrapped_modules.add(module_id)
            return True
            
        # Directly target specific module instances by name rather than by class
        if hasattr(module, '_name'):
            if module._name in ["film_generator", "value_head"]:
                wrapped_modules.add(module_id)
                return True
                
        # Explicitly check for exact module matches
        for name, child in module.named_children():
            if name in ["film_generator", "value_head"]:
                # We'll wrap the parent, not this child
                wrapped_modules.add(id(child))
                wrapped_modules.add(module_id)
                return True
                
        return False
    
    return film_module_policy_fn
